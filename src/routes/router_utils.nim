...
template showErrorHtml*(error: string; cfg: Config): string =
  ## For trusted, hardcoded HTML error messages (e.g. rate limit with links).
  renderMain(renderErrorHtml(error), request, cfg, requestPrefs(), "Error")
...
template condValidUsername*(name: string) =
  const reservedNames = ["pic", "gif", "video", "search", "settings", "login", "intent", "i", "f"]
  const validUsernameChars = {'a'..'z', 'A'..'Z', '0'..'9', '_', ','}
  cond '.' notin name
  cond name notin reservedNames
  cond name.allCharsInSet(validUsernameChars)
