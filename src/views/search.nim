    ...
    tdiv(class="search-bar"):
      form(`method`="get", action="/search", autocomplete="off"):
        ...
        input(`type`="text", name="q", autofocus="",
              placeholder="Search posts...", dir="auto")
...
proc renderSearchPanel*(query: Query): VNode =
  ...
  buildHtml(form(`method`="get", action=action,
    ...
    hiddenField("f", "tweets")
    genInput("q", "", query.text, "Search posts...", class="pref-inline")
    button(`type`="submit"): icon "search"
...
proc profileSurfaceEmptyMessage*(username: string; kind: QueryKind): VNode =
  let msg = case kind
    of replies: "No replies from @" & username & "."
    of media: "No media from @" & username & "."
    else: "No posts from @" & username & "."
  buildHtml(tdiv(class="timeline-header")):
    h2(class="timeline-none"): text msg

proc renderLocalCollectionSearchPanel*(query: Query; collectionId: string): VNode =
  let action = "/collection/" & collectionId & "/search"
  buildHtml(form(`method`="get", action=action,
                 class="search-field", autocomplete="off")):
    hiddenField("f", "tweets")
    genInput("q", "", query.text, "Search posts...", class="pref-inline")
    ...
    input(id="collection-search-panel-toggle", `type`="checkbox", checked=isPanelOpen(query))
    label(`for`="collection-search-panel-toggle"): icon "down"
