...
proc expandTextEntities(tweet: Tweet; entities: JsonNode; text: string; textSlice: Slice[int];
  ...
  with urls, entities{"urls"}:
    for u in urls:
      ...
      let expanded = u.getExpandedUrl
      let isArticleLink =
        "/i/article/" in expanded and
        ("https://x.com/" in expanded or "https://twitter.com/" in expanded)
      if tweet.articleUrl.len == 0 and "/i/article/" in expanded and
         ("https://x.com/" in expanded or "https://twitter.com/" in expanded):
        tweet.articleUrl = expanded
      if urlStr.len == 0 or urlStr notin text:
        continue

      replacements.extractUrls(u, textSlice.b, hideTwitter = hasRedundantLink or isArticleLink)
