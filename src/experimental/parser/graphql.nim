...
proc parseUserResult*(userResult: UserResult): User =
  ...
  if userResult.businessAccount.isSome:
    result.affiliatesCount = userResult.businessAccount.get.affiliatesCount

  if userResult.affiliatesHighlightedLabel.isSome:
    let label = userResult.affiliatesHighlightedLabel.get.label
    if result.affiliateBadgeName.len == 0:
      result.affiliateBadgeName = label.description
    if result.affiliateBadgeUrl.len == 0:
      result.affiliateBadgeUrl = label.badge.url
    if result.affiliateBadgeTarget.len == 0:
      result.affiliateBadgeTarget = label.url.url
